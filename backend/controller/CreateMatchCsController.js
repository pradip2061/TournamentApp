  ...
  try {
    ...
    if (!userinfo.gameName[0].freefire) {
      return res.status(400).json({
        message: "add Game Name for FreeFire in your profile",
      });
    }
  ...
  try {
    ...
    const countPlayers = (games) => {
      return games.reduce(
        (total, game) =>
          total +
          game.gameName.reduce(
            (sum, playersArray) => sum + playersArray.length,
            0
          ),
        0
      );
...
const createFreeFireFullMap = async (req, res) => {
  try {
    const userId = req.user;
    ...
    if (!playermode || !entryFee || !time) {
      return res.status(400).json({ message: "Please fill all the fields" });
    }

    const FFfullmap = await FFfreefire.create({
      playermode,
      entryFee,
      time: time,
      coustum: {
    ...
    FFfullmap.save();
    res.status(200).json({ message: "Match Card Created Successfully" });
  } catch (error) {
    console.error("Error fetching user:", error);
