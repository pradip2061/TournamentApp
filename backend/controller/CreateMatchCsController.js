...
const joinuser = async (req, res) => {
  try {
    ...
    if (!userinfo.gameName[0]?.freefire) {
      return res.status(400).json({ message: "Add Free Fire game name in your profile" });
    }

    // Ensure matchId array exists
    if (!userinfo.matchId?.FreefireClashId) {
    ...
    // Ensure teamopponent array exists
    if (!match.teamopponent || match.teamopponent.length === 0) {
      match.teamopponent = [{ userid: userid }];
    } else {
...
const getFFmatch = async (req, res) => {
  try {
   const card =  await FFfreefire.find({ status: { $in: ["pending", "running"] } },{hidden:true})
    .sort({ createdAt: -1 }); 
    res.status(200).json({
      card
    });
