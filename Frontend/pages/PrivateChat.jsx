...
const LandingChat = ({navigation, route}) => {
  ...
  useEffect(() => {
    const fetchLatestMessages = async () => {
      ...
      for (const friend of friends) {
        try {
          const roomId = ['Sagar', friend.id].sort().join('<->');
          const response = await axios.get(
            `${API_URL}/khelmela/chat/${roomId}`,
          );
          if (response.data.length > 0) {
            messages[friend.id] = response.data.sort(
              (a, b) => new Date(b.time) - new Date(a.time),
            )[0];
          } else {
            messages[friend.id] = null;
          }
        } catch (error) {
  ...
  useEffect(() => {
    ...
    sortedFriends.sort((a, b) => {
      const messageA = friendMessages[a.id];
      const messageB = friendMessages[b.id];

      if (!messageA && !messageB) return 0;
      if (!messageA) return 1;
      if (!messageB) return -1;
      return new Date(messageB.time) - new Date(messageA.time);
    });
    ...
    if (searchTerm.trim() === '') {
      ...
      const fetchFilteredFriends = async () => {
        try {
          ...
          let filtered = response.data.filter(user =>
            sortedFriends.some(friend => friend.id === user.id),
          );
  ...
  }, [searchTerm, friends, friendMessages]);
            ...
            <TouchableOpacity onPress={() => handleOnpress(item)}>
              <View style={styles.friends}>
                ...
                <View>
                  ...
                  {friendMessages[item.id] && (
                    <Text style={styles.latestMessage}>
                      {friendMessages[item.id].message}
                    </Text>
                  )}
                </View>
              </View>
